...
(defun go_web! ( ) ; Revised Dec. 2009
        (let (poss-actions step action-name old-wffs old-terms (return-str nil) )

(setq return-str (format nil "~% GO ~%"))
                ...
                (when (null poss-actions)
                      (incf *AG-clock* 1)
                      (incf *real-clock* 1)
                      (return-from go_web! (concatenate 'string return-str "NO MORE ACTIONS POSSIBLE!"))
                )
 (setq return-str (concatenate 'string return-str (format nil "~%~%POSSIBLE ACTIONS & VALUES: ~a" poss-actions)))
 (setq return-str (concatenate 'string return-str (format nil "~%SEEMINGLY BEST PLAN: ~a" *plan*)))
                ...
                (setq step (pop *plan*))
  (setq return-str (concatenate 'string return-str (format nil "~%~%STEP TO BE TAKEN: ~a" step)))
  (setq return-str (concatenate 'string return-str (format nil "~%EXPECTED STATE; ~%  ~a" (second *states*))))
                (pop *plan*)
